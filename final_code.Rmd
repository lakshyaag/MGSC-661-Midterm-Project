...
ggplot(data, aes(x = movie_budget)) +
  ...
  labs(x = "Movie budget", y = "Number of movies", title = "Distribution of movie budget1") +
  theme_pubr() +
  theme_lox()
...
data_cleaned %>%
  ...
  cor() %>%
  round(2) %>%
  corrplot(method = "number", type = "full", tl.col = "black")
...
rownames(performance) <- c("MSE", "RMSE", "Number of predictors")
...
```
From the ANOVA, we see that the spline regression model has the lowest residuals. However, when we check the predicitions, we see that the spline model actually provides a rating prediction outside the boundary of 0-10. Therefore, we decide to go with the polynomial regression model as our final model.
...
stargazer(poly_k_fold, header = FALSE, digits = 2, type = "latex", out = "tables/polynomial_regression_summary.html", title = "Polynomial regression")
```
