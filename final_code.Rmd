...
rmse_train_lin_reg_k <- sqrt(mse_train_lin_reg_k)

mse_test_lin_reg_k <- mean((predict(lin_reg_k, test) - test$imdb_score)^2)
rmse_test_lin_reg_k <- sqrt(mse_test_lin_reg_k)


print(glue("Training set MSE: {mse_train_lin_reg_k}"))
print(glue("Training set RMSE: {rmse_train_lin_reg_k}"))
print(glue("Test set MSE: {mse_test_lin_reg_k}"))
print(glue("Test set RMSE: {rmse_test_lin_reg_k}"))
```
...
```{r}
poly_k_fold <- glm(imdb_score ~ poly(movie_budget, 3) + poly(duration, 2) + poly(nb_news_articles, 5) + poly(movie_meter_IMDBpro, 5) + . - movie_budget - duration - nb_news_articles - movie_meter_IMDBpro, data = data_cleaned)

mse_poly_k_fold <- cv.glm(data_cleaned, poly_k_fold, K = 10)$delta[1]
...
spline_model_1 <- glm(
  imdb_score ~
    bs(
      movie_budget,
      knots = quantile(data_cleaned$movie_budget, c(0.25, 0.5, 0.75)),
      degree = 3
    ) +
    bs(
      duration,
...
```{r}
cv_results <- cv.glm(data_cleaned, spline_model_1, K = 10)
mse <- cv_results$delta[1]
rmse <- sqrt(mse)
r_squared <- 1 - (spline_model_1$deviance / spline_model_1$null.deviance)

print(glue("Spline MSE: {mse}"))
print(glue("Spline RMSE: {rmse}"))
print(glue("R-squared: {r_squared}"))
```
...
predictions <- data.frame(test_data$movie_title, lin_reg_predict, poly_reg_predict, spline_reg_predict)
names(predictions) <- c("movie_title", "lin_reg", "poly_reg", "spline_reg")
...
## Linear regression
```{r include=FALSE}
names(lin_reg_2$coefficients) <- c(
  "Intercept",
  "Movie Budget",
  ...
  "Plot Involves Love",
  "Plot Involves Friends",
  "Plot Involves Death",
  "Plot Involves High School",
  "Plot Involves Police",
  "Plot Set in New York City",
  "Plot Involves a Boy",
  "Plot Involves Drugs",
  "Plot Involves School",
...
# stargazer(lin_reg_2, type = "html", out = "lin_reg.html", dep.var.labels = c("IMDB Score"))
```
...
# stargazer(poly_k_fold, type = "html", out = "poly_reg.html", dep.var.labels = c("IMDB Score"))
```

## Spline regression
```{r include=FALSE}
names(spline_model_1$coefficients) <- c(
  "Intercept",
  "Movie Budget (B-spline 1)",
  "Movie Budget (B-spline 2)",
  "Movie Budget (B-spline 3)",
  "Movie Budget (B-spline 4)",
  "Movie Budget (B-spline 5)",
  "Movie Budget (B-spline 6)",
  "Duration (B-spline 1)",
  "Duration (B-spline 2)",
  "Duration (B-spline 3)",
  "Duration (B-spline 4)",
  "Duration (B-spline 5)",
  "News Articles (B-spline 1)",
  "News Articles (B-spline 2)",
  "News Articles (B-spline 3)",
  "News Articles (B-spline 4)",
  "IMDB Pro Meter (B-spline 1)",
  "IMDB Pro Meter (B-spline 2)",
  "IMDB Pro Meter (B-spline 3)",
  "IMDB Pro Meter (B-spline 4)",
  "Released in February",
  "Released in March",
  "Released in April",
  "Released in May",
...
# stargazer(spline_model_1, type = "latex", out = "spline_reg.html", dep.var.labels = c("IMDB Score"))
```
...
stargazer(
  ...
  column.labels = c("Linear", "Polynomial", "Spline"),
  digits = 3,
  out = "overall_reg.html"
)
```
