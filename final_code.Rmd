...
data_cleaned <- data_cleaned %>% mutate(
  aspect_ratio = fct_lump_n(aspect_ratio, n = 2),
  maturity_rating = fct_lump_n(maturity_rating, n = 3)
)
```
...
```{r}
top_10_distributors <- data_cleaned %>%
  select(distributor) %>%
...
```
```{r}
outliers <- c(492, 316, 395, 989, 12, 1581)
