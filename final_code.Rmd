...
spline_model_1 <- glm(
  imdb_score ~
    poly(movie_budget, 3) +
    bs(duration, knots = quantile(duration, c(0.25, 0.5, 0.75)), degree = 2) +
    bs(
      nb_news_articles,
      knots = quantile(nb_news_articles, c(0.25, 0.5, 0.75)),
      degree = 1
    ) +
    bs(
      movie_meter_IMDBpro,
      knots = quantile(movie_meter_IMDBpro, c(0.25, 0.5, 0.75)),
      degree = 1
...
```{r}
cv_results <- cv.glm(data_cleaned, spline_model_1, K = 20)
...
predictions <- data.frame(lin_reg_predict, poly_reg_predict, spline_reg_predict)
names(predictions) <- c("lin_reg", "poly_reg", "spline_reg")
